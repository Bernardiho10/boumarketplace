import { D as DataProvider, a as MagicView, g as getCoordinatesWithJitter, b as MagicViewModel } from './property-f7a6443c.js';
...
class NeuralMarketView extends MagicView {
    ...
    async enterStage(propId) {
        ...
        if (refEl)
            refEl.innerText = `REF: ${prop.refId || '—'}`;
        ...
        if (toiletsEl)
            toiletsEl.innerText = (prop.toilets || prop.bathrooms || 0).toString();
        const parkingEl = document.getElementById("stage-parking");
        if (parkingEl)
            parkingEl.innerText = (prop.parkingSpaces || 0).toString();
        ...
        const callBtn = document.getElementById("stage-call-btn");
        if (callBtn) {
    ...
    renderBentoGridData(items, container) {
        ...
        container.innerHTML = items.map((p, i) => {
            const specParts = [];
            if (p.bedrooms > 0)
                specParts.push(`${p.bedrooms} Beds`);
            if (p.bathrooms > 0)
                specParts.push(`${p.bathrooms} Baths`);
            if (p.toilets > 0)
                specParts.push(`${p.toilets} Toilets`);
