...
class ListingPage {
    ...
    render() {
        ...
        if (total)
            total.textContent = this.properties.length.toString();
        if (count)
    ...
    getFilteredProperties() {
        ...
        return this.properties.map(property => {
            ...
            if (type && !searchable.includes(type))
                return { property, score: 0 };
            if (status && property.status.toLowerCase() !== status)
                return { property, score: 0 };
    ...
    renderCard(property) {
        ...
        if (property.bedrooms > 0)
            specs.push(`${property.bedrooms} beds`);
        if (property.bathrooms > 0)
            specs.push(`${property.bathrooms} baths`);
        if (property.toilets !== undefined)
            specs.push(`${property.toilets} toilets`);
        if (property.parkingSpaces !== undefined)
            specs.push(`${property.parkingSpaces} parking`);
        if (property.sqft > 0)
    ...
    renderFeedback(resultCount) {
        ...
        if (query.length >= 3 && !isPropertySearchQuery(query)) {
            ...
            `;
            feedback.querySelectorAll(".feedback-chip").forEach(chip => {
    ...
    toNumericPrice(price) {
        return Number(price.replace(/[^\d]/g, "")) || 0;
    }
    getSourceLabel(url) {
        if (url.includes("jiji"))
            return "Jiji";
        if (url.includes("nigeriapropertycentre"))
            return "NPC";
